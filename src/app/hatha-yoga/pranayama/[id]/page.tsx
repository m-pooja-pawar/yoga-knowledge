...
export async function generateStaticParams() {
  return hathaTopics
    .filter((topic) => topic.parentId === 'pranayama')
    .map((topic) => ({ id: topic.id }));
}

export default async function PranayamaSubTopicPage({ params }: TopicPageProps) {
  const resolvedParams = await params;
  ...
  const pranayamaCount = hathaTopics.filter(t => t.parentId === 'pranayama').length;
    ...
    <div className="max-w-3xl mx-auto px-4 pt-20 pb-8 lg:pt-12 lg:pb-12 lg:pl-8">
      <header className="mb-10 text-center lg:text-left">
        <div className="inline-block bg-sage-100 text-sage-600 text-base font-medium px-3 py-1 rounded-full mb-4">
          Pranayama {topic.subOrder} of {pranayamaCount}
        </div>
